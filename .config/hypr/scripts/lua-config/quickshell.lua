...
local function call_target(monitor, target, log_file)
	...
	local cmd = string.format("qs -p '%s' ipc call '%s.%s' toggle 2>&1", M.qs_config, target, monitor)
...
function M.toggle(target, title, log_file)
	...
	utils.sleep(0.4)
	os.execute(string.format("qs -p '%s' >/tmp/quickshell-nabin.log 2>&1 &", M.qs_config))
	utils.sleep(1.2)
...
function M.toggle_control_center()
    ...
    -- Check if it's open
    local is_open, ok = utils.execute_capture(string.format("hyprctl clients -j 2>/dev/null | jq -e '.[] | select(.title == \"%s\")' 2>/dev/null", CC_TITLE))
    ...
    local monitor = utils.target_monitor()
    os.execute(string.format("qs -p '%s' ipc call 'controlPanel.%s' toggle", M.qs_config, monitor))
end
