...
log_file="/tmp/quickshell-wallpaper-ipc.log"
printf "%s\n" "wallpaper helper invoked at $(date --iso-8601=seconds)" >>"${log_file}"
...
call_target() {
  ...
  output="$(qs -p "${qs_config}" ipc call "wallpaperLauncher.${monitor}" toggle 2>&1)" || status=$?
  printf "%s\n" "target=wallpaperLauncher.${monitor} status=${status} output=${output}" >>"${log_file}"
...
notify-send "Quickshell Wallpaper" "Could not open wallpaper selector. Check ${log_file} and /tmp/quickshell-nabin.log." >/dev/null 2>&1 || true
exit 1
