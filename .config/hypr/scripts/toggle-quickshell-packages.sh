...
log_file="/tmp/quickshell-packages-ipc.log"
printf "%s\n" "packages helper invoked at $(date --iso-8601=seconds)" >>"${log_file}"
...
call_target() {
  ...
  output="$(qs -p "${qs_config}" ipc call "packages.${monitor}" toggle 2>&1)" || status=$?
  printf "%s\n" "target=packages.${monitor} status=${status} output=${output}" >>"${log_file}"
...
notify-send "Quickshell Packages" "Could not open package searcher. Check ${log_file} and /tmp/quickshell-nabin.log." >/dev/null 2>&1 || true
exit 1
