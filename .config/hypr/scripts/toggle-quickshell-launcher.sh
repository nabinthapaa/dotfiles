...
log_file="/tmp/quickshell-launcher-ipc.log"
printf "%s\n" "launcher helper invoked at $(date --iso-8601=seconds)" >>"${log_file}"
...
call_target() {
  ...
  output="$(qs -p "${qs_config}" ipc call "appLauncher.${monitor}" toggle 2>&1)" || status=$?
  printf "%s\n" "target=appLauncher.${monitor} status=${status} output=${output}" >>"${log_file}"
...
notify-send "Quickshell Launcher" "Could not open application launcher. Check ${log_file} and /tmp/quickshell-nabin.log." >/dev/null 2>&1 || true
exit 1
